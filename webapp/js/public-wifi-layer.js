      ...
      try {
        ...
        if (!map.getLayer(LAYER_ID)) {
          ...
          map.on("click", LAYER_ID, (e) => {
            ...
            const offlineNote = !isOnline() ? '<div style="font-size:10px;color:#92400e;margin-bottom:6px">Offline mode</div>' : "";
            ...
            const html =
              ...
              '<div style="font-weight:700;font-size:14px;color:#1e293b;margin-bottom:4px">WiFi ' +
              (p.name || "Public WiFi") +
              "</div>" +
    ...
    try {
      ...
      if (wantOn) {
        ...
        if (!data || !data.features || !data.features.length) {
          toast(isOnline() ? "Could not load WiFi data for this city yet" : "Offline and no cached WiFi data");
          enabled = false;
        ...
        setVisible(map, true);
        connectWs();
        toast(isOnline() ? "Public WiFi on" : "Public WiFi on (offline)");
      } else {
        setVisible(map, false);
        disconnectWs();
