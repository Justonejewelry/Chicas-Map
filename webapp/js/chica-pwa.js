...
(function (global) {
  ...
  function loadSiteChrome() {
    if (!document.getElementById("chicaSiteNavCss")) {
      var css = document.createElement("link");
      css.id = "chicaSiteNavCss";
      css.rel = "stylesheet";
      css.href = "css/site-nav.css?v=nav1";
      document.head.appendChild(css);
    }
    if (!document.getElementById("chicaSiteNavJs")) {
      var script = document.createElement("script");
      script.id = "chicaSiteNavJs";
      script.src = "js/site-nav.js?v=nav1";
  ...
  function markExitShown() {
    ...
    try { localStorage.setItem(EXIT_KEY, String(Date.now())); } catch (_) {}
  }
  ...
  function wireExitIntent() {
    ...
    document.addEventListener("visibilitychange", function () {
      if (document.visibilityState === "hidden") openExitThanks();
    });
  }
