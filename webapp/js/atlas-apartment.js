/* Apartment Friday packet — counts only, no names. */
(function () {
  ...
  function persist() {
    try {
      localStorage.setItem("atlas-apt", JSON.stringify({ p: $("prop").value, z: $("zips").value }));
    } catch (e) {}
  }
  function restore() {
    try {
      var s = JSON.parse(localStorage.getItem("atlas-apt") || "{}");
  ...
  function load() {
    ...
    Promise.all(list.map(function (zip) {
      ...
      $("note").textContent = "Three different City files. Do not add them together.";
