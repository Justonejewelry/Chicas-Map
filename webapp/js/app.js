(function () {
  ...
  function popupHtml(s) {
    ...
    const dirs = directionsUrl(s);
    const sniff = s._sniff != null ? `<div class="popup-meta">Sniff ${s._sniff}/5 🦴</div>` : "";
    ...
    ul.innerHTML = items
      .map((s, i) => {
        ...
        <div class="row">
          ...
          ${s._sniff ? `<span class="pill sniff" title="Cheeks Sniff Score">${s._sniff}/5 🦴</span>` : ""}
          ${s._early ? `<span class="pill early">Early</span>` : ""}
          ${s._wish ? `<span class="pill wish">Match</span>` : ""}
        </div>
      </li>`;
      })
      .join("");
  ...
  function makeMarkerEl(s) {
    ...
    if (s._sniff && s._sniff >= 4) el.dataset.sniff = s._sniff;
    el.title = (s.title || s.address || "Sale") + (s._miles != null ? ` · ${formatMiles(s._miles)}` : "") + (s._sniff ? ` · sniff ${s._sniff}` : "");
    return el;
  }

  function renderMarkers() {
    clearMarkers();
    sponsorMarkers.forEach((m) => m.remove());
    sponsorMarkers = [];
  ...
  function renderDnaHeat() {
    if (!window.ChicaFeatures || !map) return;
    if (!dnaHeatOn) {
      ChicaFeatures.removeHeatLayer(map);
      return;
    }
    try {
      const geo = ChicaFeatures.heatFeatures(allSales(), (feed && feed.hot_zones) || []);
      if (map.isStyleLoaded()) ChicaFeatures.ensureHeatLayer(map, geo);
      else map.once("load", () => ChicaFeatures.ensureHeatLayer(map, geo));
    } catch (_) {}
  }

  function renderCameo() {
    if (!window.ChicaFeatures || !map) return;
    const top = ChicaFeatures.pickCameo(filtered());
    if (!top || top.lat == null) return;
    const el = document.createElement("div");
    el.className = "yb-cameo";
    el.title = "Cheeks was here · top sniff";
    el.innerHTML = "🐕";
    cameoMarker = new maplibregl.Marker({ element: el, offset: [0, -28] })
      .setLngLat([top.lon, top.lat])
      .setPopup(new maplibregl.Popup().setHTML(`<b>Cheeks cameo</b><br/>${esc(top.title || top.address)}<br/>Sniff ${top._sniff || "?"}/5`))
      .addTo(map);
  }

  async function renderSponsorPins() {
    if (!window.ChicaFeatures || !map) return;
    const list = await ChicaFeatures.loadSponsors(city);
    list.forEach((sp) => {
      if (sp.lat == null || sp.lon == null) return;
      const el = document.createElement("div");
      el.className = "yb-sponsor-pin";
      el.title = sp.name;
      el.textContent = "★";
      const html = `<div class="popup-title">${esc(sp.name)}</div>
        <div class="popup-meta">${esc(sp.pitch || "")}</div>
        ${sp.url ? `<div class="popup-meta"><a href="${esc(sp.url)}" target="_blank" rel="noopener">Visit ↗</a></div>` : ""}
        <div class="popup-meta">Partner pin · ${esc(sp.tier || "local")}</div>`;
      const marker = new maplibregl.Marker({ element: el })
        .setLngLat([sp.lon, sp.lat])
        .setPopup(new maplibregl.Popup({ offset: 16 }).setHTML(html))
        .addTo(map);
      sponsorMarkers.push(marker);
    });
  }
  ...
  async function loadCity(slug) {
    ...
    if (city === "texas") {
      ...
      const ws = document.getElementById("weatherStrip");
  ...
  function wireFeaturesUI() {
    ...
    document.getElementById("btnFirst30")?.addEventListener("click", () => {
      ...
      refresh();
      toast(first30Only ? "First 30 mode — early + high sniff" : "Showing all hours");
    });
    ...
    document.getElementById("btnWishAdd")?.addEventListener("click", () => {
      ...
      if (!input || !window.ChicaFeatures) return;
      ChicaFeatures.addWishlistItem(input.value);
      input.value = "";
      renderWishlist();
      toast("Hunting list updated");
    });
    document.getElementById("btnWishFilter")?.addEventListener("click", () => {
      wishlistOnly = !wishlistOnly;
      document.getElementById("btnWishFilter")?.classList.toggle("active", wishlistOnly);
      refresh();
      toast(wishlistOnly ? "Only wishlist matches" : "All sales");
    });
    document.getElementById("btnShareRoute")?.addEventListener("click", async () => {
      ...
      const stops = routeIds.map((k) => byKey[k]).filter(Boolean);
      if (!stops.length) {
        toast("Add stops to your route first");
        return;
      }
      const text = ChicaFeatures.buildRouteStory(stops, CITY_META[city]?.name);
      const ok = await ChicaFeatures.shareRouteStory(text);
      toast(ok ? "Route card ready" : "Could not share");
    });
  ...
  function renderWishlist() {
    ...
    if (!el || !window.ChicaFeatures) return;
    const list = ChicaFeatures.getWishlist();
    if (!list.length) {
      el.innerHTML = '<li class="empty">Add tools, records, cameras…</li>';
      return;
    }
    el.innerHTML = list
      .map(
        (w) =>
          `<li><span>${esc(w)}</span> <button type="button" data-w="${esc(w)}" class="icon-btn">×</button></li>`
      )
      .join("");
    el.querySelectorAll("[data-w]").forEach((btn) => {
      btn.addEventListener("click", () => {
        ChicaFeatures.removeWishlistItem(btn.dataset.w);
        renderWishlist();
        if (wishlistOnly) refresh();
      });
    });
  }
